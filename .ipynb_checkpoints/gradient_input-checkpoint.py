...
class GradientInput():
    ...
    def gradient_input(self, index, ratio = 0.5, step = 1000, learning_rate = 5e-4):
        """
        ...
        losses = []
        A = self.gradient_tape(A,Y_target,mask_id,step, learning_rate) # get new A
        
        self.plot_new_predictions(A_orig,A ,A_Date,y_Date)
    ...
    def gradient_tape(self, A,Y_target,mask_id=False,step = 1000, learning_rate = 5e-4):
        """
        ...
        for i in tqdm(range(step)):
            with tf.GradientTape() as tape:       
                ...
                if i > 0 and loss_record[-1] >= loss_record[-2]: break
    ...
    def plot_new_predictions(self,A_orig,A, A_Date, y_Date):
        ...
        plt.scatter([dt.strptime(x, "%d/%m/%Y") for x in A_Date.values], 
                    prediction_painter.y_inverse_scaler(daily_cases_X.T,self.scaler),s=150,edgecolors='black',color='white')
        plt.scatter([dt.strptime(x, "%d/%m/%Y") for x in A_Date.values], 
                    prediction_painter.y_inverse_scaler(daily_cases_X.T,self.scaler), label = 'Inputs',s=60,edgecolors='black')
        plt.scatter([dt.strptime(x, "%d/%m/%Y") for x in y_Date.values], 
                    prediction_painter.y_inverse_scaler(y_orig[0],self.scaler), label = 'Original predictions',s=80,edgecolors='black',color = 'lightgrey')
        plt.scatter([dt.strptime(x, "%d/%m/%Y") for x in y_Date.values], 
                    prediction_painter.y_inverse_scaler(y_after[0],self.scaler), label = 'Target predictions',s=80,edgecolors='black',color = 'grey')
        plt.legend()
